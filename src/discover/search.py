...
# Optional gpu imports for cuda and mps
try:
    import cupy as cp
except ImportError:
    cp = None
...
try:
    ...
    # minimal torch definition for type checking if not available
    class torch_dummy:
        @staticmethod
...
def _format_feature_str(sisso_instance, feature_name):
    ...
    if sym_expr and hasattr(sisso_instance, 
                            'sym_clean_to_original_map_') and sisso_instance.sym_clean_to_original_map_:
...
class ParametricModel:
    def __init__(self, sym_expr, primary_symbols):
        ...
        self._predict_func = sympy.lambdify(self.primary_symbols_str, 
                                            self.sym_expr, 'numpy')
    ...
    for i, base_feat_name in enumerate(base_feature_names):
        for op_name in parametric_ops:
            ...
            def objective_func(params):
                ...
                nonlin_params = params[len(initial_linear_coefs):]
                ...
                return np.sqrt(mean_squared_error(y.values, y_pred, 
            ...
            with warnings.catch_warnings():
                ...
                result = minimize(objective_func, x0, method='L-BFGS-B', 
                                  bounds=bounds, options={'maxiter': nlopt_max_iter})
            ...
            if result.success and np.isfinite(result.fun) and result.fun < refined_model['score']:
                ...
                temp_sym_features[i] = op_def['sym_func'](base_sym_features[i], 
                                                          p_symbols)
                ...
                refined_model['model'] = ParametricModel(final_expr, 
                                                    sisso_instance.primary_symbols_)
                refined_model['coef'] = None 
...
def _find_best_models_sa(sisso_instance, phi_sis_df, y, D_max, task_type,
    ...
    for D in range(1, D_max + 1):
        ...
        for i in range(iterations):
            ...
            new_score, model_data = _score_single_model(phi_pruned[new_features], 
                                                        y, task_type, model_params, 
                                                        sample_weight, device, 
                                                        torch_device)
        ...
        final_score, final_model_data = _score_single_model(final_X_df, y, task_type, 
                                                        model_params, sample_weight, 
                                                        device, torch_device)
        
        sym_features = [sisso_instance.feature_space_sym_map_.get(f, 
                            sympy.sympify(f)) for f in best_features_for_D]
        models_by_dim[D] = {
...
def _find_best_models_rmhc(sisso_instance, phi_sis_df, y, X_df, D_max, task_type,
    ...
    for D in range(1, D_max + 1):
        ...
        sym_features = [sisso_instance.feature_space_sym_map_.get(f, 
                                        sympy.sympify(f)) for f in best_features_for_D]
        models_by_dim[D] = {
            'features': best_features_for_D, 'score': final_score, 
            'model': final_model_data.get('model'),
            'coef': final_model_data.get('coef'), 
...
def _find_best_models_miqp(sisso_instance, phi_sis_df, y, D_max, task_type,
    ...
    for D_iter in range(1, D_max + 1):
        ...
        try:
            ...
            sym_features = [sisso_instance.feature_space_sym_map_.get(f, 
                                    sympy.sympify(f)) for f in final_feature_names]
            models_by_dim[len(final_feature_names)] = {
                'features': final_feature_names, 'score': score, 
                'model': model_data.get('model'),
                'coef': model_data.get('coef'), 'sym_features': sym_features, 
                'is_parametric': False
            }
...
def _score_ch_combo(new_feature, base_features, phi_df, y, task_type, 
                    model_params, sample_weight):
...
def _find_best_models_ch_greedy(sisso_instance, phi_sis_df, y, D_max, task_type,
    ...
    for D_iter in range(1, D_max + 1):
        ...
        tasks = (delayed(_score_ch_combo)(new_feat, selected_features, phi_pruned, 
                                    y, task_type, model_params, 
                                    sample_weight) for new_feat in remaining_features)
...
def _find_best_models_brute_force(sisso_instance, phi_sis_df, y, D_max, task_type,
    ...
    for D_iter in range(1, D_max + 1):
        ...
        tasks = (delayed(_score_brute_force_combo)(combo, phi_pruned, y, task_type, 
                                                   model_params, sample_weight, device, 
                                                   torch_device) for combo in combos)
        results = Parallel(n_jobs=sisso_instance.n_jobs, prefer="threads")(tasks)
        ...
        sym_features = [sisso_instance.feature_space_sym_map_.get(f, 
                                            sympy.sympify(f)) for f in best_combo]
        models_by_dim[D_iter] = {
...
def _find_best_models_greedy(sisso_instance, phi_sis_df, y, X_df, D_max, task_type,
    ...
    for D_iter in range(1, D_max + 1):
        ...
        X_current_dim_df = phi_pruned[selected_features]
        score, model_data = _score_single_model(X_current_dim_df, y, task_type, 
                                                model_params, sample_weight, device, 
                                                torch_device)
        ...
        current_sym_features = [sisso_instance.feature_space_sym_map_.get(f, 
                                        sympy.sympify(f)) for f in selected_features]
        models_by_dim[D_iter] = {
            'features': selected_features.copy(), 'score': score, 
            'model': model_data.get('model'),
            'coef': model_data.get('coef'), 'sym_features': current_sym_features, 
            'is_parametric': False
        }
...
def _find_best_models_sisso_pp(sisso_instance, phi_sis_df, y, D_max, task_type,
    ...
    if final_model is not None:
        ...
        models_by_dim[1] = {'features': [feature_names[i] for i in best_d1_info['indices']], 
                            'score': final_score,
                            'model': final_model, 'coef': final_coef, 
                            'sym_features': sym_features, 'is_parametric': False}
    ...
    for D_iter in range(2, D_max + 1):
        ...
        for model_info in best_models_at_level:
            ...
            for new_feat_idx in range(prev_indices[-1] + 1, n_features):
                ...
                next_level_candidates.append({'proxy_score': new_proxy_score, 
                                              'rss': new_total_rss, 
        ...
        if final_model is not None:
            ...
            models_by_dim[D_iter] = {'features': [feature_names[i] for i in best_d_iter_info['indices']], 
                                    'score': final_score, 'model': final_model, 
                                    'coef': final_coef, 'sym_features': sym_features, 
                                    'is_parametric': False}
